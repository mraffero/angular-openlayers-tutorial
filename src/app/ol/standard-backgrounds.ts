...
export class StandardBackground {
  ...
  static createBaseLayers(options: any) {
    const layers = options.layers
    const olLayers = [];
    for (const layer of layers) {
      console.debug(layer);

      olLayers.push(new olLayerTile({
        source: new csiwmts({
          url: layer.sourceOptions.url,
          wmtsLayers: layer.sourceOptions.layer,
        }),
        title: layer.title,
        baseLayer: layer.baseLayer,
        visible: layer.visible,
        preview:
          `https://geomap.reteunitaria.piemonte.it/mapproxy/service?layer=${layer.sourceOptions.layer}&style=default&tilematrixset=grid_32632_nw&Service=WMTS&Request=GetTile&Version=1.0.0&Format=image%2Fpng&TileMatrix=06&TileCol=31&TileRow=32`
      })
      )
